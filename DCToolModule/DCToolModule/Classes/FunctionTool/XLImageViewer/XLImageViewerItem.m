...
@interface XLImageViewerItem ()<UIScrollViewDelegate,UIActionSheetDelegate>
{
    //ScrollView
...
-(void)buildUI{
    ...
    NSString *isSave = [[NSUserDefaults standardUserDefaults] objectForKey:@"PFSHOUXINSAVEPHOTO"];
    if ([isSave isEqualToString:@"isSave"]) {
        //添加长按保存
...
- (void) longPress:(UILongPressGestureRecognizer *)longPress {
    ...
    UIActionSheet *sheet = [[UIActionSheet alloc]initWithTitle:@"选择保存" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"保存图片", nil];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (!buttonIndex) {
        if (_imageView) {
            ...
            [self ProgressHUDWithTitle:@"图片已保存"];
        }
    }
}

- (void)ProgressHUDWithTitle:(NSString *)title{
...
-(void)setImageUrl:(NSString *)imageUrl{
    ...
    if (_showDataImage) {
        ...
        if ([_imageUrl isKindOfClass:[UIImage class]]) {
            _imageView.image = (UIImage *)_imageUrl;
        }else{
    ...
    //显示本地图片
    if (!_showNetImage) {
        _imageView.image = [self Base64StrToUIImage:_imageUrl];
        [self setImageViewFrame];
        [_loading hide];
        return;
    }
    ...
    if ([isSave isEqualToString:@"isSave"]) {
        ...
        UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(longPress:)];
        [self addGestureRecognizer:longPress];
        placeholderStr = @"PFShouXinBigBeiJing";
    }
    
    //显示网络图片
    [_imageView sd_setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:[UIImage imageNamed:placeholderStr] options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
...
-(void)showShrinkDownAnimation{
    ...
    //设置CollectionView透明度
    _collectionView.backgroundColor = [UIColor colorWithWhite:0 alpha:1];
    NSLog(@"zhi'qian%@",NSStringFromCGRect(_imageView.frame));
    [UIView animateWithDuration:0.35 animations:^{
        _collectionView.backgroundColor = [UIColor colorWithWhite:0 alpha:0];
